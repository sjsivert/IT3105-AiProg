...
class Critic(nn.Module):
    ...
    def __init_model(self, nn_dimensions: list, activation_functions: list, optimizer: str, loss_funciton: str) -> None:
        ...
        for x in range(1, len(nn_dimensions)):
            ...
            self.model.add_module(f'A{x-1}', activation_func)
    ...
    def update_state_value(self, td_error: float, reinforcement: float, state: list, next_state: list) -> None:
        ...
        # Modify gradients
        gradients = self.model.parameters()
        for index in range(gradients):
            gradients[index].grad *= 0.5 / td_error
            self.eligibilities[index] += gradients[index].grad
            gradients[index].grad = self.eligibilities[index] * td_error
    ...
    @staticmethod
    def __convert_to_tensors(state: list, next_state: list, discount_factor: float, reinforcement: float):
        state = torch.as_tensor(state)
