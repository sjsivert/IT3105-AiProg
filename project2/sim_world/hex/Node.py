...
class Peg(Node):
    def __init__(self, location, pegValue=0):
        super().__init__()
        self.location = location
        self.neighboursDic = {}
        # Value=0 -> node is empty
        # Value=1 -> player 1 has a peg there
        # Value=-1 -> player -1 has a peg there
    ...
    def addBiDirectionalNeighbour(self, node, location: str) -> bool:
        ...
        invertedLocation = str(-int(locationList[0])) + \
            "," + str(-int(locationList[1]))
        if node in self.neighboursDic.values() and node != self:
            return False
        self.neighboursDic[location] = node
